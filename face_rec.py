...
# # redis-10188.c10.us-east-1-2.ec2.redns.redis-cloud.com:10188
# # Yn2rsNluRIiyaviucORUIxlMq5M7mJCg
hostname = 'redis-10188.c10.us-east-1-2.ec2.redns.redis-cloud.com'
portnumber = 10188
password = 'Yn2rsNluRIiyaviucORUIxlMq5M7mJCg'
...
#     retrive_df.columns=['name_role','facial_features']
#     retrive_df[['Name','Role']]=retrive_df['name_role'].apply(lambda x: x.split('@')).apply(pd.Series)
#     #retrive_df = retrive_df[
#     #     retrive_df['facial_features'].apply(lambda arr: arr.shape == (512,))
...
# def ml_search_algorithm(dataframe, feature_column, test_vector,
#                         name_role =['Name', 'Role'], thresh=0.5):
#     # using cosine similarity based search algorithm 
...
#     def saveLogs_redis(self):
#         #step 1 create a log of dataframe
#         dataframe = pd.DataFrame(self.logs)
...
#         encode_data = []
#         for name, role, ctime in zip(name_list, role_list, ctime):
#             if name != 'Unknown':
#                 concat_string = f"{name}@{role}@{ctime}"
#                 encode_data.append(concat_string)
...
#         #step -2 use for loop and extract each embedding and pass thru to the ml search algorithm
#         for res in results:
#             x1, y1, x2, y2 = res['bbox'].astype(int)
#             embeddings = res['embedding']
...
#     def get_embedding(self,frame):
#         #get results from insightface model
#         results = faceapp.get(frame,max_num=1) #setting the max num to 1 cause we only want to get details of one person only 
#         embeddings = None
#         for res in results: 
#             self.samples += 1
#             x1, y1, x2, y2 = res['bbox'].astype(int)
